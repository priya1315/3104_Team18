...
class HDVilaDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.data_dir = '/content/FollowYourPose/data'
            ...
            for row in reader:
                if row['clip_id'] != last_clip_id:
                    ...
                    if count % self.all_rank == self.global_rank:
                        self.metadata.append([(row['part_id'])+","+row['clip_id']]) 
    ...
    def _get_video_path(self, sample):
        part_id = sample[0].split(",")[0]
        clip_id = sample[0].split(",")[1]
        video_path = os.path.join(self.data_dir, part_id, clip_id)
        return video_path
    ...
    def __getitem__(self, index):
        ...
        #caption = sample[0][int(cap_idx//64)]
